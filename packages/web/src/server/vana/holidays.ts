/** US holiday calendar for Vana's week briefings — deterministic, no table, no API.
 ...
 *  If the Flutter app ever needs this, promote it to a `holidays` table seeded from here. */
...
/** Source of truth: the `holidays` table (seeded by 20260831000000_create_holidays.sql, extendable
 *  per country_code). Falls back to the computed calendar when the table is empty/unreadable —
 *  e.g. dates beyond the seeded window. */
export async function holidaysInRange(startIso: string, endIso: string): Promise<Holiday[]> {
  try {
    const { dbAny } = await import("./env");
    const { data, error } = await dbAny().from("holidays").select("holiday_date, name, federal").eq("country_code", "US").gte("holiday_date", startIso).lte("holiday_date", endIso).order("holiday_date");
