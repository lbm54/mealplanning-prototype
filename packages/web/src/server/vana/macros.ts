/** Daily targets always come from the app's `calculate-daily-macros` edge function — never
 *  recomputed here. When the current week has missing days in daily_macro_targets (nobody
 *  opened the app to compute them), we call the deployed service with the same payload the
 *  Flutter app builds (daily_macro_service.dart) and cache the result the same way. */
import { dbAny, today, addDays } from "./env";

const MACROS_FN = `${process.env.SUPABASE_URL ?? ""}/functions/v1/calculate-daily-macros`;
const MACROS_KEY = process.env.SUPABASE_SERVICE_ROLE_KEY ?? process.env.SUPABASE_ANON_KEY ?? "";
...
export async function ensureWeekTargets(userId: string, anchorDate?: string): Promise<void> {
  ...
  try {
    const { data: have } = await d.from("daily_macro_targets").select("target_date").eq("user_id", userId).gte("target_date", ws).lte("target_date", addDays(ws, 6));
    const missing = week.filter((dt) => !(have ?? []).some((r) => r.target_date === dt));
    if (!missing.length) return;

    const { data: u } = await d.from("users").select("gender, birthday, weight_pounds, height_feet, height_inches, body_fat_pct, lifestyle, typical_weekly_hours, carb_cycle_opt_in, training_phase").eq("id", userId).maybeSingle();
    if (!u?.weight_pounds) return; // no profile → nothing the service can compute from
    const { data: actsRaw } = await d.from("activities").select("id, scheduled_date_time, title, activity_type, duration_minutes, intensity_level, tss, intensity_z1_z2_pct, intensity_z3_z4_pct, intensity_z5_pct, distance_miles, pace_target_minutes_per_mile, cycling_speed_mph, swimming_pace_per_100m_seconds").eq("user_id", userId).is("deleted_at", null).gte("scheduled_date_time", addDays(ws, -1)).lt("scheduled_date_time", addDays(ws, 8)).order("scheduled_date_time");
    ...
    const res = await fetch(MACROS_FN, { method: "POST", signal: AbortSignal.timeout(15_000), headers: { Authorization: `Bearer ${MACROS_KEY}`, apikey: MACROS_KEY, "content-type": "application/json" },
    ...
    if (!res.ok) throw new Error(`calculate-daily-macros ${res.status}`);
    ...
    const rows = out.map((r, i) => ({ id: crypto.randomUUID(), user_id: userId, target_date: missing[i],
    ...
    const { error } = await d.from("daily_macro_targets").upsert(rows, { onConflict: "user_id,target_date" });
    if (error) throw new Error(error.message);
  } catch (e) {
    attempted.set(key, Date.now()); // back off; context falls back to whatever is cached
    console.warn("[macros] ensureWeekTargets failed:", (e as Error).message);
  }
}
