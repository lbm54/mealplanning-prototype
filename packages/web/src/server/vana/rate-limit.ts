...
export async function checkRateLimit(userId: string, fn: RateLimitedFn): Promise<{ allowed: boolean; retryAfterSeconds?: number }> {
  const w = WINDOWS[fn];
  try {
    const since = new Date(Date.now() - w.seconds * 1000).toISOString();
    const { count, error } = await dbAny().from("vana_calls").select("*", { count: "exact", head: true }).eq("user_id", userId).like("function_name", `${fn}%`).gte("created_at", since);
    if (error || count == null) return { allowed: true };
    return count >= w.max ? { allowed: false, retryAfterSeconds: w.seconds } : { allowed: true };
  } catch { return { allowed: true }; }
}
/** Thrown by model paths that have no HTTP response of their own (opener, brief, embeddings); routes map it to 429. */
...
export async function assertRateLimit(userId: string, fn: RateLimitedFn): Promise<void> {
  const r = await checkRateLimit(userId, fn);
