...
import type { MealPlan, ShoppingItem } from "@/lib/vana/contracts";
...
async function pantryFromLogs(userId: string): Promise<Set<string>> {
  const since = new Date(Date.now() - 30 * 86400_000).toISOString().slice(0, 10);
  const { data } = await dbAny().from("meal_logs").select("items").eq("user_id", userId).eq("is_deleted", false).gte("log_date", since).limit(200);
...
export async function buildShoppingList(userId: string, plan: MealPlan): Promise<ShoppingItem[]> {
  const d = dbAny();
  ...
  for (const m of plan.meals) {
    if (m.source === "library" && m.libraryMealId) {
      const { data } = await d.from("meal_library").select("ingredients_json").eq("id", m.libraryMealId).maybeSingle();
      ...
      const { data } = await d.from("saved_meals").select("items").eq("id", m.savedMealId).maybeSingle();
  ...
  return buildItems(resolved, await pantryFromLogs(userId));
}
